...
{
    assert(key);
    return strnlen(key, MAX_WORD_LEN);
}
...
hash_t RolHash(elem_t key)
{
    ...
    for (int i = 0; i < MAX_WORD_LEN; ++i)
    {
        ...
        hash = (hash << 1) | (hash >> (sizeof(hash_t)*8 - 1));
        hash ^= key[i];
    }
...
{
    hash_t crc = 0xFFFFFFFFUL;

    while (*key)
        crc = Crc32Table[(crc ^ *(key++)) & 0xFF] ^ (crc >> 8);
...
{
    #define PROCESS_8SYM(x)  "crc32   %0, qword ptr [%1 + " #x "]\n\t"
    #define PROCESS_16SYM(x) PROCESS_8SYM(x)   \
                             PROCESS_8SYM(x+8) 
